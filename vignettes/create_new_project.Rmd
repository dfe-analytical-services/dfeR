...
Considerations = c("Where do you want to store your project?. You should store any projects that use Git outside of OneDrive. To                       create a folder in your C drive, type: 'C:/Users/username' into the file explorer search bar and create a folder                     called 'repos'.",
                  ...
                  "Should the folder structure be for a publication project? Set to TRUE if yes.", 
                  "Boolean (TRUE/FALSE)")
)
...
### Example
Using the create_project function from the dfeR package, you can quickly set up a new R project with a custom folder structure. By leveraging RStudio and renv, you can ensure your projects are well-organised and reproducible
```{r, eval = FALSE}
# Load the dfeR package
library(dfeR)
# Create a new project with the desired parameters
dfeR::create_project(
  path = "C:/Users/MSmith/repos/my_new_project",
  init_renv = TRUE,
  include_structure_for_pkg = FALSE,
...
The above code will create a new project in the specified folder path, with renv initialised, a publication project structure, and a GitHub gitignore file included. You can then open the project in RStudio and start working on your analysis or publication.
