...
3.  **Customisation**:  Other files or processes can be initialised as well depending on whether you set extra arguments as `TRUE` or `FALSE`. For example, you can use customisation to create a GitHub .gitignore file or initialise the `renv` package.
...
parameter_data <- data.frame(
  ...
  Output = c(
    ...
    "If set to `TRUE`, an extra package development folder will be created",
