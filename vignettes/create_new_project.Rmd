...
You can create a project using the `create_project()` function using code or using the project wizard in RStudio. The project wizard is a user-friendly way to set up a new project without writing any code. This vignette covers both methods. 
...
parameter_data <- data.frame(
  ...
  Output = c(
    ...
    "If set to `TRUE` or selected, renv will be initialised in your project.",
    "If set to `TRUE` or selected, an extra package development folder will 
be created",
    "If set to `TRUE` or selected, extra publication specific folders will
be created",
    "If set to `TRUE` or selected, a GitHub .gitignore file will be created."
  ),
  `Expected input` = c(
    "A folder path e.g., 'C:/Users/user name/repos'",
...
kable(
  ...
  column_spec(c(1, 3), width = "5em") %>%
  column_spec(c(2, 4), width = "7em") %>%
...
![](../man/figures/create_new_project_screenshot.png){fig-alt="Newly created contents of project folder in file explorer."}
...
If you are using RStudio, you can also create a new project using the project wizard. This is a user-friendly way to set up a new project without writing any code. To do this, follow these steps:

1.  Open RStudio and go to the 'File' menu.
2.  Select 'New Project' from the dropdown menu.
3.  Choose 'New Directory' and then scroll down to find 'dfeR project template'.
4.  Enter the name of your project under 'Directory name' and choose a location to save it.
5.  Decide whether you want to have a project structure specifically for publications or a package. Leaving these both unticked will give you a project with a generic analysis project structure. You then need to decide whether you would like a GitHub .gitignore file. 
6.  Click 'Create Project' to set up your new project.


![](../man/figures/project_wizard_screenshot.png){fig-alt="New Project Wizard with option to create dfeR project template open."}

You can also access the project wizard by clicking on the project that you are currently in, in the top-right hand corner and then selecting 'new project'.
