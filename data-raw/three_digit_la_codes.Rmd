...
This is documentation that shows the process used to add the three digit local authority codes to the wd_pcon_lad_la_rgn_ctry look up. It contains a step by step guide followed by the code used throughout the process.
...
2.  Go to the [Explore Education Statistics screener repository](https://github.com/dfe-analytical-services/dfe-published-data-qa/blob/main/data/las.csv), download the 'las.csv' file.
    ...
    1.  Save this file in the 'data' folder of the dfeR repo. This step is temporary.

3.  Carry out the necessary data processing and check if there is any data missing from the GIAs tables that we can obtain from the screener file.
...
8.  Update the [contributing documentation](https://dfe-analytical-services.github.io/dfeR/CONTRIBUTING.html) and package using the workflow in package documentation.
...
wd_pcon_lad_la_rgn_ctry <- dfeR::wd_pcon_lad_la_rgn_ctry %>%
  dplyr::left_join(
    ...
    by = c("la_name" = "la_name", "new_la_code" = "new_la_code")
  ) %>%
  # if there are NA values in old_la_code, change them to 'z'
  dplyr::mutate(old_la_code = dplyr::if_else(is.na(old_la_code), "z", old_la_code))
