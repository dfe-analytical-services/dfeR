...
- `write_df_to_delta()` now accepts `DATABRICKS_HOST` with or without a scheme; bare hosts have `https://` prepended automatically, and `http://` is upgraded to `https://`. An unset or empty `DATABRICKS_HOST` now produces a clearer error.
